/** Mechanical score helpers for the Gpt-6 Astra batch. No shared melodies. */
export { writeSong, up } from './lib.js';
export const CREDIT = 'Created by Gpt-6 Astra · 2026-09-23';
...
export function pattern(name, rows, channels, pans = [112, 128, 152, 128, 176]) {
  channels.forEach((ch, c) => {
    // No inherited tail in a rest, even when seeking directly into a section.
    if (!ch[0]?.note) ch[0] = { ...ch[0], note: '^^' };
    const fx = `X${pans[c].toString(16).toUpperCase().padStart(2, '0')}`;
    // Preserve J/SC; pan on the first free effect row instead.
    let r = 0;
    while (ch[r]?.fx) r++;
